...
public class NewCardActivity extends BaseActivity implements View.OnClickListener,NewCardListAdapter.OnNewCardAddCallBack{
                ...
                case HttpUtil.SUCCESS:
                    ...
                    if (msg.arg1 == RequestCode.DEALADDCARD){
                        ...
                        if (model.getResult().equals("1")){
                            mList.get(currPos).setResult(String.valueOf(opinion));
                            adapter.notifyDataSetChanged();
                            ToastUtil.showBottomShort(mContext,"结果处理成功");
                        }else{
    ...
    public void onIgnore(int pos) {
        currPos = pos;
        opinion = 2;
        dealAddCard(mList.get(pos).getCardID(),String.valueOf(opinion));
    }
    ...
    public void onEntry(int pos) {
        ...
        if (MyApplication.loginType==1){
            intent.putExtra("personID",mList.get(pos).getUserID());
        }else{
            intent.putExtra("companyID",mList.get(pos).getUserID());
        }
