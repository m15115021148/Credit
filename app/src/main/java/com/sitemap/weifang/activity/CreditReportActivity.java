...
//        mSubmitText.setText("提交");
        ...
        mCreditName.setText(MyApplication.loginModel.getXm());
        String codeNum = MyApplication.loginModel.getCode();
        String str = codeNum.substring(codeNum.length()-6,codeNum.length()-2);
        mCreditCode.setText(MyApplication.loginModel.getCode().replace(str,"****"));
        mCreditName.setFocusable(false);
        mCreditName.setEnabled(false);
        mCreditName.setClickable(false);
        mCreditCode.setFocusable(false);
        mCreditCode.setEnabled(false);
        mCreditCode.setClickable(false);
    }
    ...
    public void onClick(View v) {
        ...
        if (v == mSubmit) {//提交
            ...
            if (TextUtils.isEmpty(mCreditResult.getText().toString())){
                ToastUtil.showBottomShort(mContext, mResult.getText().toString()+"不能为空");
                return;
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == IMG_RESULT_CODE){
                try{
            ...
            if(requestCode == IMG_RESULT_PHONE_CODE){
                try {
                    Uri uri = data.getData();
                    String imageAbsolutePath = imgUtil.getImageAbsolutePath(uri);
                    Log.e("result", imageAbsolutePath);
