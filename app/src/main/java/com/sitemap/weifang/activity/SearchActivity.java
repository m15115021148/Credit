...
//                            }else{
//                                ToastUtil.showBottomShort(mContext,"没有查到相关信息");
//                            }
//                        }else{
                            List<SearchCompanyModel> list = JSONObject.parseArray(msg.obj.toString(), SearchCompanyModel.class);
                                Intent intent = new Intent(mContext,SearchResultActivity.class);
                                intent.putExtra("SearchCompanyModel", JSON.toJSONString(list));
                                startActivity(intent);
...
//                http.sendGet(RequestCode.QUERYDATA, WebUrlConfig.getQueryPerson(xm, code));
    ...
    public void onClick(View v) {
        ...
        if (v == mSearch){
            if (TextUtils.isEmpty(name.getText().toString().trim())&&TextUtils.isEmpty(code.getText().toString().trim())){
                ToastUtil.showBottomShort(mContext,"查询条件不能全为空");
                return;
            }
            if (MyApplication.loginType==1) {//个人登录
...
//                    return;
//                }
//                if (!RegularUtil.isInputCharOrNum(code.getText().toString())) {
//                    ToastUtil.showBottomShort(mContext, mCode.getText().toString()+"输入不规范");
//                    return;
//                }
            }else{
                if (!RegularUtil.isInputCharOrNum(code.getText().toString())) {
            ...
            checkData(MyApplication.loginType,name.getText().toString(),code.getText().toString());
        }
    }
}
